...
const EditDuration = (props) => {
    ...
    const [dateOfDuration, setDateOfDuration] = useState(props.route.params?.startingDate);
    const [fromTimeModal, setFromTimeModal] = useState(false);
    const [fromTime, setFromTime] = useState(props.route.params?.startTime);
    const [toTime, setToTime] = useState(props.route.params?.endTime);
    ...
    const [repeatDaily, setRepeatDaily] = useState({
        startingDate: props.route.params?.startingDate,
        repeat: 'Daily',
        every: '1',
        end: props.route.params?.endDate
    });

    const [repeatWeekly, setRepeatWeekly] = useState({
        startingDate: props.route.params?.startingDate,
        repeat: props.route.params?.daysOfWeek,
        every: '1',
        on: [],
        end: props.route.params?.endDate
    });
    ...
    function next() {
        ...
        if (dateOfDuration == "" && repeat?.startingDate == "") {
        ...
        apiRequest(body, route)
            .then(data => {
                setLoading(false);
                if (data.status) {
                    alertRefInner.alertWithType('success', 'Updated');
                    setTimeout(() => {
