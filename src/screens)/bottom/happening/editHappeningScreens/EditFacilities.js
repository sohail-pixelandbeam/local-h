...
const EditFacilities = (props) => {
    ...
    const faciltiesProps = props.route.params?.Facilities;
    console.log('faciltiesProps', faciltiesProps)
    const [selectedFacilties, setSelectedFacilties] = useState(faciltiesProps);
    ...
    function checkAdditionalFacilities() {
        const titles = facilitesArr.map(item => item.title);
        let arr = facilitesArr;
        faciltiesProps.forEach(item => {
            const title = item
            if (!titles.includes(title)) {
                arr.push({ title: title });
            }
        });
        setFacilitiesArr(arr)
        forceUpdate();
            ...
            <View style={styles.contentContainer}>
                <ScrollView contentContainerStyle={{ paddingBottom: 150 }} >
                                        ...
                                        <TouchableOpacity
                                            ...
                                            <View style={{ width: 54, height: 54, borderRadius: 54 / 2, borderWidth: selectedFacilties.includes(v?.title) ? 0 : 2, backgroundColor: selectedFacilties.includes(v.title) ? '#5B4DBC' : 'white', borderColor: 'rgba(0,0,0,0.4)', alignItems: 'center', justifyContent: 'center' }}>
                                                {Icon ? <Icon color={selectedFacilties.includes(v.title) ? "#fff" : "#222222"} />
                                                    :
                                                    <Text style={{ fontFamily: fonts.PMe, fontSize: 30, color: selectedFacilties.includes(v.title) ? 'white' : '#222222', marginTop: 5, }}>{v.title.charAt(0)}</Text>
                                                }
                                            </View>
                    ...
                    <TouchableOpacity
                        ...
                        <View style={{ width: 25, height: 25, borderRadius: 25 / 2, backgroundColor: 'white', alignItems: 'center', justifyContent: 'center', elevation: 2, shadowColor: 'grey', shadowOpacity: 0.4, shadowOffset: { width: 1, height: 1 } }}>
                    ...
                    <TouchableOpacity
                        onPress={() => next()}
                        style={{ width: "50%", height: 47, borderRadius: 20, backgroundColor: '#5B4DBC', alignItems: 'center', justifyContent: 'center', marginTop: 40, alignSelf: 'center' }}>
