...
security create-keychain -p "lh-1234" $KEYCHAIN_PATH/build.keychain
security default-keychain -s $KEYCHAIN_PATH/build.keychain
security import ./.github/secrets/Certificates.p12 -t agg -k $KEYCHAIN_PATH/build.keychain -P "$IOS_KEYS" -A
...
security list-keychains -s $KEYCHAIN_PATH/build.keychain
#security default-keychain -s ~/Library/Keychains/build.keychain
security default-keychain -s $KEYCHAIN_PATH/build.keychain
#security unlock-keychain -p "lh-1234" ~/Library/Keychains/build.keychain
security unlock-keychain -p "lh-1234" $KEYCHAIN_PATH/build.keychain

#security set-key-partition-list -S apple-tool:,apple: -s -k "lh-1234" ~/Library/Keychains/build.keychain
security set-key-partition-list -S apple-tool:,apple: -s -k "lh-1234" $KEYCHAIN_PATH/build.keychain
